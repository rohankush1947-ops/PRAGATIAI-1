    ...
    <div className="space-y-8">
      ...
      <div className="p-6 sm:p-8 rounded-2xl bg-gradient-to-r from-purple-50 via-white to-indigo-50 border border-purple-200/90 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6">
        <div>
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-xs text-slate-600 font-medium">Professor of Transportation & Deep-Tech AI, IIT Madras</span>
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div className="p-5 rounded-2xl bg-white border border-slate-200 shadow-sm">
          ...
          <span className="text-[11px] text-slate-500 mt-1 block">Scored across 5 criteria</span>
        </div>

        <div className="p-5 rounded-2xl bg-white border border-slate-200 shadow-sm">
          <span className="text-xs text-slate-500 block mb-1">Active Pilots Monitored</span>
          <div className="text-3xl font-black text-sky-700 font-mono">
            {activePilotsCount}
          </div>
          <span className="text-[11px] text-slate-500 mt-1 block">Field trial oversight</span>
        </div>
      </div>
      ...
      <div className="p-6 sm:p-8 rounded-2xl bg-white border border-slate-200 shadow-sm space-y-4">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
          ...
          <Link
            to="/expert/challenges"
            className="text-xs font-semibold text-purple-700 hover:text-purple-600 flex items-center gap-1 shrink-0"
          >
                ...
                {challenges.slice(0, 6).map((ch) => (
                  ...
                  >
                    <td className="py-3 px-3">
                      <span className="font-mono font-bold text-purple-800 bg-purple-50 px-2 py-0.5 rounded border border-purple-200 text-xs">
                        {ch.id}
                      </span>
                    </td>
                    <td className="py-3 px-3 max-w-sm">
                      <div className="font-bold text-slate-900 text-xs hover:text-purple-700 transition-colors">
                        {ch.title}
                      </div>
                      <p className="text-[11px] text-slate-600 truncate max-w-xs mt-0.5">
                    ...
                    <td className="py-3 px-3 text-right">
                      <div className="flex items-center justify-end gap-1.5">
                        <Link
                          ...
                          className="px-2.5 py-1 bg-purple-50 hover:bg-purple-100 border border-purple-300 text-purple-800 rounded text-[11px] font-semibold inline-flex items-center gap-1 transition-colors"
                        >
      ...
      <div className="p-6 sm:p-8 rounded-2xl bg-white border border-slate-200 shadow-sm space-y-4">
        <div className="flex items-center justify-between border-b border-slate-200 pb-3">
          ...
          <span className="text-xs text-purple-700 font-bold">Standard 100-Point Rubric</span>
        </div>
            ...
            <tbody className="divide-y divide-slate-200">
              {evaluations.map((ev) => (
                <tr key={ev.id} className="hover:bg-slate-50 transition-colors">
                  <td className="py-3 px-3">
                    <div className="font-bold text-slate-900">{ev.startupName}</div>
                    <span className="text-[10px] text-slate-500">Ref: {ev.applicationId}</span>
                  </td>
                  <td className="py-3 px-3 max-w-xs">
                    <div className="text-slate-800 truncate font-medium">{ev.challengeTitle}</div>
                  </td>
                  <td className="py-3 px-3 text-slate-600">
                    {ev.date}
                  </td>
                  <td className="py-3 px-3">
                    <span className="font-bold font-mono text-emerald-800 bg-emerald-50 px-2 py-0.5 rounded border border-emerald-300 text-xs">
                      {ev.totalScore}/100
                    </span>
                  </td>
                  <td className="py-3 px-3">
