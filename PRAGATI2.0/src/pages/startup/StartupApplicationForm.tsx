  ...
  return (
    <div className="max-w-4xl mx-auto space-y-8">
      ...
      <div className="p-5 rounded-2xl bg-[#0B1528] border border-slate-800 space-y-2">
        <span className="text-[10px] font-bold uppercase tracking-wider text-sky-400">{selectedChallenge.department}</span>
        <h3 className="text-base font-bold text-white">{selectedChallenge.title}</h3>
        <p className="text-xs text-slate-300 leading-relaxed">{selectedChallenge.problemDescription}</p>
        <div className="pt-2 flex flex-wrap items-center gap-4 text-xs text-slate-400">
          <span>Budget Range: <strong className="text-slate-200">{selectedChallenge.budgetRange}</strong></span>
          <span>•</span>
          <span>Pilot Duration: <strong className="text-slate-200">{selectedChallenge.pilotDuration}</strong></span>
        </div>
      </div>

      {/* FORM */}
      <form onSubmit={handleSubmit} className="p-6 sm:p-8 rounded-2xl bg-[#0B1528] border border-slate-800 shadow-xl space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              ...
              disabled
              className="gov-input opacity-80 cursor-not-allowed"
            />
          </div>
        ...
        <div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
            {formData.documents.map((doc, idx) => (
              <div key={idx} className="flex items-center justify-between p-2.5 rounded-lg bg-[#070E1E] border border-slate-800 text-xs text-slate-300">
                <div className="flex items-center gap-2 truncate">
                  <Paperclip className="w-3.5 h-3.5 text-sky-400 shrink-0" />
                  <span className="truncate">{doc}</span>
                </div>
                <span className="text-[10px] text-emerald-400 font-bold shrink-0">Attached</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="pt-4 border-t border-slate-800 flex items-center justify-between">
          ...
          <button
            type="submit"
            className="px-6 py-2.5 bg-sky-600 hover:bg-sky-500 text-white rounded-xl text-xs font-bold flex items-center gap-2 shadow-lg shadow-sky-950 transition-all"
          >
